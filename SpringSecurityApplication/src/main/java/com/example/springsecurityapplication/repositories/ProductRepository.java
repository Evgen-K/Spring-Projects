...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    // Поиск всех продуктов по части наименования продукта с игнорированием регистра
    ...
    // *** Поиск  и фильтрация по диапазону цены, сортировка по возрастанию цены,  а также фильтрация по категории
    @Query(value = "select * from product where category_id = ?3 and (price >= ?1 and price <= ?2) order by price",nativeQuery = true)
    List<Product> findByAndCategoryOrderByPriceAsc(float ot, float Do, int category);
    ...
    // *** Поиск и фильтрация по диапазону цены, сортировка по убыванию цены,  а также фильтрация по категории
    @Query(value = "select * from product where category_id = ?3 and (price >= ?1 and price <= ?2) order by price desc",nativeQuery = true)
    List<Product> findByAndCategoryOrderByPriceDesc(float ot, float Do, int category);
}
